...
### Added
- **설정 자동 백업** (준호님 요청) — 설정 변경 시 드라이브의 설정 백업 파일(`BrainBloom_settings.json`, 항상 1개 유지)을 자동 갱신. 조건: 초기 로드 완료 + 드라이브 연결 + 새 토글 `settingsAutoSync`(기본 켬, 설정 → 저장·드라이브에서 끔 가능). 3초 디바운스로 연속 변경을 1회 저장으로 묶음.

### Technical Notes
- 수동 '설정 저장' 버튼과 자동 백업이 **공용 코어 `saveSettingsToDriveCore`** 를 공유(로직 드리프트 방지). 문서 저장(`isSavingRef`)과 겹치면 해당 회차 건너뜀(다음 변경 때 자연 재시도).
- 정직 고지: 자동 백업 실패는 사용자 흐름을 막지 않도록 **콘솔 경고만** 남기고 조용히 넘어감(백업 성격 + 다음 변경 때 재시도). '불러오기'는 여전히 수동 버튼 — 다른 기기의 로컬 설정을 자동으로 덮지 않는 것이 의도된 설계.
- 부수: 설정 백업 안내 문구의 낡은 "(자동 아님)" 표기 갱신.
- 변경 파일: `index.html`, `data/bb-data.js`(RECENT_CHANGES), `seahyun/brainstorm_v3.112.0.html`(자족 스냅샷), `CHANGELOG.md`.
...
- **텍스트 패널 라벨 수정 시 가지 메타 전멸** — `preserveMetadata`를 '라벨 경로' 전역 키 방식에서 **부모 짝 안 2단 매칭**(① 라벨+동명순번 — 순서 이동에 강함 ② 같은 형제 자리 폴백 — 라벨 변경 구제)으로 재작성. 조상이 개명돼도 후손 이관이 끊기지 않고, 관계선/경계/그룹 재매핑도 옛 id→새 id 직접 매핑으로 단순화. 잔여 한계(라벨 변경+순서 이동 동시 수행 시 새 노드 취급)는 코드 주석에 명시.
- **sanitize 무통보 절삭** — `_sanitizeTruncated` 플래그 + `sanitizeTreeTruncated()` 신설. 절삭(깊이 100/노드 2만/라벨 5천자/노트 4천자) 발생 시 시작 복원·문서 전환·붙여넣기·JSON 열기·드라이브 불러오기·가져오기 6개 경로에서 경고 토스트. **시작 복원은 자동저장이 절삭본을 덮어쓰기 전에 원본 raw를 `lastWork_truncated_backup`에 1회 백업**.
- **다중 키 쓰기 원자화(중복 탭 피해 완화)** — `idbSetMany`(한 트랜잭션) 신설, `writeActiveDoc`이 lastWork(미러)+doc 슬롯+docMeta를 원자적으로 기록 + 목록 검증을 미러 쓰기보다 앞으로(삭제된 문서의 대기 디바운스가 미러를 오염시키던 문제). 문서 전환의 빈 슬롯 분기도 미러 즉시 동기화. 중복 탭 배너 문구에 덮어쓰기 위험 명시.

### Technical Notes
- 정직 고지: 두 탭이 **같은 문서를 동시에 편집**하는 경우의 나중-저장-승리(last-writer-wins)는 이번 범위에서 해결되지 않음 — 단일 writer(Web Locks) 도입은 별도 과제로 남김. 이번 수정은 "미러/목록 불일치로 인한 교차 오염·문서 증발"의 구조적 창을 닫는 것.
- `preserveMetadata` 재작성은 동작 변화가 있는 수정 — 기존 대비 보존이 늘어나는 방향(악화 케이스 없음: 종전 매칭되던 라벨+순번 케이스는 ①에서 동일하게 매칭됨).
- 변경 파일: `index.html`, `seahyun/brainstorm_v3.106.0.html`(스냅샷), `CHANGELOG.md`.
...
- **작업량·비용 메타 소실** — 우측 패널은 문자열 저장인데 `sanitizeNode`가 숫자만 통과시켜 붙여넣기·재시작 복원·드라이브/JSON 불러오기에서 소실 → 문자열도 허용(200자 제한).
- **설정 백업이 가짜 "다른 기기 최신본" 유발** — `checkRemoteNewer`만 `SETTINGS_FILE_NAME` 필터 누락(다른 3개 호출부는 있음) → 필터 추가. "설정 저장" 직후 자동저장 보류·수동 저장 차단되던 문제 해소.
- **검색창 연 채 편집 시 선택 강제 점프** — 검색 effect가 tree 변경마다 첫 매치로 setSelectedId+스크롤+깜빡임 → `searchJumpRef`로 "검색어/창 변경 때만 점프, 트리 변경 땐 목록 재계산+현재 매치 인덱스 유지"로 분리.
- **자동저장 실패 알림 클릭 시 이동 무효** — 드라이브 섹션이 display:none인 탭 상태에서 scrollIntoView가 no-op → `setCat('save')` 선행.
- **undo/redo 후 콜레싱 미리셋** — 직후 800ms 내 타이핑이 이전 묶음에 병합돼 undo 단계 소실 → 리셋 추가.
- **자동저장 실패 경고 30초 소등** — 토큰 갱신/핑 성공이 경고를 끄던 것 제거, 해제는 실제 저장 성공 시에만(실패 저장은 내용이 다른 한 자동 재시도되므로 성공 시 자연 해제).
- **몰입 모드 편집 잠금 우회** — 날짜/시간 삽입 단축키(Ctrl+; · Shift+Alt+D/T)가 몰입 모드에서 라벨을 수정 → 가드 추가.
- **무변경 편집의 히스토리 오염** — F2 후 그대로 Enter/Esc여도 '라벨 변경' 스냅샷+updatedAt 갱신 → 무변경이면 편집 종료만(★샘플★ 확정은 예외 유지).
- **SW 오프라인 기동 실패(쿼리 URL)** — `cache.match`가 쿼리까지 정확 일치 요구 → 폴백을 ignoreSearch → '/' → '/index.html' 순으로 확장 (`sw.js`).

### Technical Notes
- 버그 감사(4개 병렬 에이전트, 2026-08-10)에서 확정된 20건 중 1차분. 상위 2건(작업량 소실·설정 파일 필터)은 코드 직접 재검증 후 수정.
- 변경 파일: `index.html`, `sw.js`, `seahyun/brainstorm_v3.104.2.html`(스냅샷), `CHANGELOG.md`.
...
### Changed
- **설정 메뉴 목적별 재구성** (준호님 승인 초안대로) — 탭 6개 유지하되 구성 변경:
  - 🖥 **화면**: 기존 14개+ 항목이 뒤섞여 있던 "화면 표시" 단일 섹션을 **🎨 테마·배경**(배경 색 7종·점 패턴) / **🧩 화면 구성**(미니멀 상단바·좌우 패널·Undo/Redo 버튼·캔버스 힌트) / **🗺 지도 동작**(자동 맞춤·노드 간격·편집 중 집중 모드) 3개 카드로 분해
  - 🔤 **노드** *(신설)*: 📏 노드 표시(줄 수 + **메타데이터 표시** 이동) / 🔠 노드 폰트 / ⭐ 아이콘 줄 구성(화면에서 이동)
  - ✍️ **작업** (구 '작업·콘텐츠'): 🌅 시작 시 동작 / **⏱ 타이머(타이머 탭 흡수)** / 💬 오늘의 격언 / 💡 새 노드 단어 / **🔔 알림(업데이트·중복 탭 — 화면에서 이동)**
  - ☁️ 저장·드라이브: **💾 내보내기**(구 '저장 옵션') / 구글 드라이브 · 🔗 연동, ℹ️ 정보(맨 뒤로) 기존 유지
- 탭 순서: 화면 → 노드 → 작업 → 저장·드라이브 → 연동 → 정보 (기본 탭은 '화면' 유지)

### Technical Notes
- `SETTINGS_CATS`에서 `timer` 제거·`node` 추가, CSS 매핑(`.settings-catbody[data-cat]`)도 `cat-timer`→`cat-node` 교체. `cat-timer` 참조 0개 확인.
- **설정 키·데이터는 전혀 변경 없음** — JSX 배치만 이동(토글의 checked/onChange 로직 원문 그대로). 기존 사용자 설정값 100% 보존.
- 이동 블록: 배경 테마 팔레트(섹션 말미→테마·배경 카드 상단), 아이콘 줄 편집 UI(→노드 탭), showMetaPanel(→노드 표시), updateNotify/duplicateTabAlert(→알림 카드), 타이머 섹션(→작업 탭, 시작 시 동작 다음).
- 검증: babel 컴파일 통과(JSX 구조 검증 겸), 카테고리별 카드 수 확인(disp 3·node 3·work 5).
- 변경 파일: `index.html`, `seahyun/brainstorm_v3.102.0.html`(스냅샷), `CHANGELOG.md`.
...
### Fixed
사용자 제보(연결선 없이 떠 있는 노드 + 지우면 다른 노드가 같이 삭제) — 원인은 **트리 안 노드 id 중복**:
- **중복 id 자가 치유** — 신규 전역 `healDuplicateIds(root)`: idSeed를 트리 최대치로 올린 뒤 겹치는/빈 id를 뒤의 것부터 재발급(콘솔 경고). `restoreWork`(모든 문서 로드·전환·복원 통과점)와 **텍스트 재파싱 직후**에 호출 — 기존 오염 문서도 **다음 로드 때 자동 수리**. 재파싱 경로는 idSeed 정합도 겸해 1000+ 노드 문서에서 새 노드 id가 재파싱 id(n1..nN)와 겹칠 수 있던 이론상 구멍도 봉합.
- **`removeNode` 단일 삭제** — `filter(c => c.id !== id)`(같은 id 전부 제거)를 `findIndex+splice`(정확히 1개)로 교체 — 만에 하나 중복이 남아 있어도 관계없는 노드까지 지워지지 않음.
- (참고) `sanitizeTree` 경유 경로는 기존 자체 방어가 있었으나 `restoreWork` 직행 경로(시작 복원 등)는 무방비였음 — 이번에 전 경로 커버.

### Technical Notes
- 변경 파일: `index.html`, `seahyun/brainstorm_v3.95.1.html`(스냅샷), `CHANGELOG.md`.
...
### Changed
- **태그별 노드 수 표시** — 하단 태그 필터 바와 우측 "사용한 태그" 목록의 각 태그 칩에 **그 태그가 붙은 노드 수**를 배지로 표시(`tagCounts` 메모 — 트리 변경 시에만 재계산).

### Technical Notes
- 변경 파일: `index.html`, `seahyun/brainstorm_v3.94.1.html`(스냅샷), `CHANGELOG.md`.
...
### Added
- **태그 라이브러리(사용한 태그 목록)** — 우측 패널 태그 입력칸 아래에 **문서 전체에서 쓴 태그 목록**이 표시된다:
  - **칩 클릭** = 지금 노드에 추가/제거(토글, 현재 노드에 붙어 있으면 강조 테두리) — 같은 태그를 다시 타이핑할 필요 없음
  - **✎** = 태그 이름을 **모든 노드에서 한 번에 변경**(새 이름이 이미 있는 노드는 중복 없이 병합, 활성 태그 필터도 따라감)
  - **✕** = **모든 노드에서 삭제**(확인 후, 활성 필터에서도 제거) — 둘 다 히스토리 1회라 `Ctrl+Z` 한 번에 복원

### Technical Notes
- 기존 `allTags` 메모(문서 전체 수집·가나다순) 재사용. 신규 `renameTagEverywhere`/`deleteTagEverywhere`(walk 일괄 적용 + `setTagFilter` 정합). `.panel-tag.lib` 칩 변형(본체 `.t`=토글, `.op`=관리 버튼).
- 변경 파일: `index.html`, `seahyun/brainstorm_v3.94.0.html`(스냅샷), `CHANGELOG.md`, `README.md`.
...
### Added
사용자 피드백 3건 반영:
- **우클릭 메뉴에서 노드 순서 이동** — 노드 우클릭 메뉴에 `🔼 위로 이동 / 🔽 아래로` 버튼 추가(같은 레벨 안, `Alt+↑/↓`와 동일한 `moveSibling`). **메뉴가 닫히지 않아 연속 이동 가능.**
- **좌/우 패널 폭 드래그 조절** — 패널 경계 안쪽 7px 핸들(`.panel-resizer`)을 끌어 폭 조절(좌 220~560px, 우 220~480px), **더블클릭=기본 폭 복원**. 그리드 컬럼을 CSS 변수(`--left-w`/`--right-w`)로 바꾸고, 드래그 중엔 DOM에 직접 써서(리렌더 0회) 부드럽게, 놓을 때 `settings.leftPanelW/rightPanelW`로 1회 커밋 — 새로고침 후에도 유지.
- **다중 선택 공통 속성 일괄 편집** — `Shift+클릭` 다중 선택 배너(기존 그룹 만들기)에 **14색 일괄 변경** 점들과 **☐ 할 일 일괄 지정/☑ 해제** 토글 추가(`bulkUpdateSelected`, 히스토리 1회로 Undo 한 번에 복원).

### Fixed
- 우클릭 메뉴 화면 하단 잘림 보정값을 새 항목 높이에 맞게 갱신(348→392px).

### Technical Notes
- 변경 파일: `index.html`, `seahyun/brainstorm_v3.93.0.html`(스냅샷), `CHANGELOG.md`, `README.md`.
...
### Fixed
집중 코드 감사 4라운드 — **데이터 정합 3건**:
- **[MED] 빈 라벨 노드가 텍스트 왕복에서 소실** — `treeToText`가 빈 라벨을 공백-만 있는 줄로 출력 → `parseInput`의 빈 줄 필터가 제거해 **노드가 사라지고 자식이 위 계층으로 승격**됐다(루트가 빈 라벨이면 전체 구조 붕괴). `treeToMarkdown`과 동일하게 `(이름 없음)` 자리표시자로 내보내고 `parseInput`이 빈 라벨로 복원.
- **[MED] 동명 형제의 메타 뒤바뀜** — `preserveMetadata`가 `경로|라벨` 키를 써서 같은 이름의 형제가 마지막 하나로 뭉개졌다(색·메모·태그·아이콘·관계선·경계선이 엉뚱한 형제에 붙음). 키에 **동명 형제 발생 순번(#k)** 을 추가해 순서대로 1:1 매칭(라벨이 유니크하면 #0 = 기존과 완전 동일 → 재정렬 회귀 없음).
- **[LOW] 노드 폭(w) 보존 무효** — `preserveMetadata`의 apply엔 `old.w` 복원 코드가 있는데 collect가 `w`를 수집하지 않아 항상 무효였다(텍스트 편집 후 사용자 지정 노드 폭 소실). 수집 목록에 `w` 추가.

### Technical Notes
- 변경 파일: `index.html`, `seahyun/brainstorm_v3.86.7.html`(스냅샷), `CHANGELOG.md`.
...
### Fixed
집중 코드 감사 2라운드 — **내보내기·동기화 4건**:
- **[HIGH] 접힌 가지가 있으면 SVG/PDF/JPG 내보내기 손상** — `buildDiagramSVG`가 `walk`(전체 순회)로 크기·연결선·노드를 그렸는데, `layoutTree`는 보이는 노드만 좌표를 계산하므로 한 번도 펼치지 않은 접힌 하위는 `_x/_w`가 없어 **NaN 크기의 깨진 SVG**(래스터는 0×0)가 되고, 펼쳤다 접은 하위는 낡은 좌표로 **숨긴 노드가 그대로 그려졌다**. 화면 렌더러와 동일하게 `walkVisible` + `_w||NODE_W`/`nodeHeight` 폴백으로 교체(접힌 노드의 숨은 자식 연결선도 제외).
- **[MED] CSV 수식 주입 무방비** — `=HYPERLINK(...)` 같은 라벨이 그대로 내보내져 Excel/Sheets에서 수식으로 실행될 수 있었다. `=`,`+`,`-`,`@`,탭,CR로 시작하는 셀에 `'` 접두 + CR도 인용 처리.
- **[MED] 충돌검사 재진입 시 '확인 중'을 '충돌 없음'으로 응답** — `checkRemoteNewer`가 이미 실행 중이면 `false`(=안전)를 반환해, 포커스 복귀 검사와 자동저장이 겹치는 순간 자동저장이 검사를 건너뛰고 원격 최신본을 덮어쓸 수 있었다. `'busy'` 반환으로 구분(자동저장은 그 주기를 건너뛰고, 수동 저장은 안내 후 중단).
- **[MED] 자동저장 중복 실행 창** — `isSavingRef`를 원격 검사 `await` 뒤에 세워, 검사가 오래 걸리면 다음 tick/수동 저장이 겹쳐 중복 버전 파일·Backup 재배치 경합이 가능했다. 검사~저장 전 구간을 조기 점유(finally 해제)로 봉합.

### Technical Notes
- 변경 파일: `index.html`, `seahyun/brainstorm_v3.86.5.html`(스냅샷), `CHANGELOG.md`.
